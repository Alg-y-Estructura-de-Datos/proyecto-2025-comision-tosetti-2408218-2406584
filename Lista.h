...
class Lista {
    ...
    public:
        ...
        int buscarPosicion(const T& dato);
};
...
// Implementación del método buscarPosicion
template <class T>
int Lista<T>::buscarPosicion(const T& dato) {
    Nodo<T>* aux = inicio;
    int pos = 0;

    while (aux != nullptr) {
        if (aux->getDato() == dato) {  // Importante: T debe tener definido operator==
            return pos;
        }
        aux = aux->getSiguiente();
        pos++;
    ...
    // Si no se encuentra el dato, se lanza excepción 404
    throw 404;
}
